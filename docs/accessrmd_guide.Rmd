...
and [contribute on
GitHub](https://github.com/r-leyshon/access_rmd_docs/issues).
...
Reads an Rmd file, converting the YAML header to a format that is
...
report](https://wave.webaim.org/report#/https://r-leyshon.github.io/access_rmd_docs/docs/accessrmd/test.html).
...
[Click to view the WAVE
check](https://wave.webaim.org/report#/https://r-leyshon.github.io/access_rmd_docs/docs/accessible_images.html).
...
See the output file [some_file.Rmd](docs/some_file.Rmd).
...
See the output file [more_parameters.Rmd](docs/more_parameters.Rmd).

You can also use a floating toc (recommended).

```{r}
access_rmd("toc_float.Rmd", title = "A Floating Toc Document", lan = "en",
           toc = "float", force = TRUE)
```

See the output file [toc_float.Rmd](docs/toc_float.Rmd).
...
Check links within an Rmarkdown document for any urls that respond with an
error.
...
Searches (recursively) directories for Rmarkdown files. Returns the found Rmd
relative paths.
...
rmds
```
This vector of Rmarkdown pathnames can be used for various operations.
...
I recurse over 37 Rmarkdown documents, using the
...
<a id="cran" href="https://cran.r-project.org/web/packages/submission_checklist.html" target="_blank">Checklist
for CRAN submissions</a> <br> <br>
<a id="ukgov" href="https://www.gov.uk/service-manual/helping-people-to-use-your-service/understanding-wcag" target="_blank">UK
